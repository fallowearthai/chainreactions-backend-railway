...
class SupabaseService {
    ...
    async testConnection() {
        try {
            ...
            const dbError = (0, ErrorHandler_1.createDatabaseError)(`Database connection test failed: ${error.message}`, 'SELECT datasets', {});
            return {
                success: false,
                error: dbError
            };
    ...
    async findDatasetMatches(searchText, searchLocation, options) {
        try {
            const startTime = process.hrtime();
            ...
            const dbError = (0, ErrorHandler_1.createDatabaseError)(`Dataset matching query failed: ${error.message}`, 'find_dataset_matches_optimized', { search_text: searchText });
            return {
                success: false,
                error: dbError
            };
    ...
    async findDatasetMatchesWithLocation(searchText, searchLocation, options) {
        try {
            const startTime = process.hrtime();
            const { CountryNormalizer } = await Promise.resolve().then(() => __importStar(require('../utils/CountryNormalizer')));
        ...
        `)
            ...
            `)
                    ...
                    if (!regionalError && regionalMatches) {
                        regionalMatches.forEach((entry) => {
                            // Avoid duplicates
                            const isDuplicate = allMatches.some(existing => existing.organization_name === entry.organization_name &&
          ...
          `)
            ...
            const dbError = (0, ErrorHandler_1.createDatabaseError)(`Geographic dataset matching failed: ${error.message}`, 'SELECT dataset_entries (geographic)', { search_text: searchText, location: searchLocation });
            return {
                success: false,
                error: dbError
            };
    ...
    processMatchEntry(entry, searchText, searchLower, geographicBoost) {
    ...
    async findDatasetMatchesOptimized(searchText, searchLocation, options) {
        try {
            const startTime = process.hrtime();
            const searchLower = searchText.toLowerCase().trim();
            const allMatches = [];
        ...
        `)
            ...
            if (exactMatches && exactMatches.length > 0) {
                exactMatches.forEach((entry) => {
                    ...
                    else if (searchQueriesLower.some(queryLower => orgNameLower.replace(/\s+/g, '') === queryLower.replace(/\s+/g, ''))) {
                        matchType = 'fuzzy';
                        confidence = 0.9;
                    }
                    // Core match (remove parentheses and spaces) - check all variations
                    else if (searchQueriesLower.some(queryLower => orgNameLower.replace(/\s*\([^)]*\)/g, '').replace(/\s+/g, '') === queryLower.replace(/\s*\([^)]*\)/g, '').replace(/\s+/g, ''))) {
                        matchType = 'core_match';
                        confidence = 0.8;
                    }
                    // Partial match - check all variations
                    else if (searchQueriesLower.some(queryLower => orgNameLower.includes(queryLower) || queryLower.includes(orgNameLower))) {
                        matchType = 'partial';
                        confidence = 0.6;
                    }
                    allMatches.push({
          ...
          `)
                ...
                if (partialMatches && partialMatches.length > 0) {
                    partialMatches.forEach((entry) => {
                        ...
                        if (searchQueriesLower.some(queryLower => orgNameLower.replace(/\s+/g, '') === queryLower.replace(/\s+/g, ''))) {
                            matchType = 'fuzzy';
                            confidence = 0.9;
                        }
                    ...
                    if (aliasMatches && aliasMatches.length > 0) {
                        aliasMatches.forEach((entry) => {
                            ...
                            if (entry.aliases && Array.isArray(entry.aliases)) {
                                const hasMatch = entry.aliases.some((alias) => alias && alias.toLowerCase() === queryLower);
                                if (hasMatch) {
            ...
            const dbError = (0, ErrorHandler_1.createDatabaseError)(`Optimized dataset matching failed: ${error.message}`, 'SELECT dataset_entries (optimized)', { search_text: searchText });
            return {
                success: false,
                error: dbError
            };
    ...
    async findDatasetMatchesFallback(searchText) {
        try {
            const startTime = process.hrtime();
        ...
        `)
            ...
            const dbError = (0, ErrorHandler_1.createDatabaseError)(`Fallback dataset matching failed: ${error.message}`, 'SELECT dataset_entries', { search_text: searchText });
            return {
                success: false,
                error: dbError
            };
    ...
    determineMatchType(searchText, orgName, aliases) {
    ...
    async getActiveDatasets() {
        try {
            ...
            const dbError = (0, ErrorHandler_1.createDatabaseError)(`Failed to get active datasets: ${error.message}`, 'SELECT datasets', { is_active: true });
            return {
                success: false,
                error: dbError
            };
    ...
    async getDatasetEntries(datasetId, limit = 100) {
        try {
            const startTime = process.hrtime();
            const { data, error } = await this.client
                .from('dataset_entries')
            ...
            const dbError = (0, ErrorHandler_1.createDatabaseError)(`Failed to get dataset entries: ${error.message}`, 'SELECT dataset_entries', { dataset_id: datasetId, limit });
            return {
                success: false,
                error: dbError
            };
    ...
    async findDatasetMatchesBatch(searchTexts) {
        try {
            const startTime = process.hrtime();
            const results = {};
            ...
            const dbError = (0, ErrorHandler_1.createDatabaseError)(`Batch dataset matching failed: ${error.message}`, 'batch_find_dataset_matches', { search_texts: searchTexts.length + ' entities' });
            return {
                success: false,
                error: dbError
            };
    ...
    async getDatabaseStats() {
        try {
            ...
            const dbError = (0, ErrorHandler_1.createDatabaseError)(`Failed to get database statistics: ${error.message}`, 'SELECT COUNT(*)', {});
            return {
                success: false,
                error: dbError
            };
