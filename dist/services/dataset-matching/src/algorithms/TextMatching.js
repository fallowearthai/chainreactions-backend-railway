...
class TextMatching {
     ...
     */
    static normalizedLevenshteinDistance(s1, s2) {
        if (s1 === s2)
            return 1.0;
        if (!s1 || !s2)
            return 0.0;
        ...
        if (maxLength === 0)
            return 1.0;
        const distance = (0, fastest_levenshtein_1.distance)(s1, s2);
        return 1 - (distance / maxLength);
    }
    /**
     * N-gram similarity calculation
     */
    static nGramSimilarity(s1, s2, n = 2) {
        if (s1 === s2)
            return 1.0;
        if (!s1 || !s2)
            return 0.0;
     ...
     */
    static calculateLengthRatio(str1, str2) {
        if (!str1 || !str2)
            return 0;
     ...
     */
    static calculateWordCountRatio(str1, str2) {
        if (!str1 || !str2)
            return 0;
     ...
     */
    static determineMatchType(searchTerm, entityName, aliases = []) {
        const normalizedSearch = EntityNormalization_1.EntityNormalization.normalizeText(searchTerm);
        ...
        if (aliases && aliases.length > 0) {
            const normalizedAliases = aliases.map(alias => EntityNormalization_1.EntityNormalization.normalizeText(alias));
            // Exact alias match
            if (normalizedAliases.includes(normalizedSearch)) {
                return 'alias';
            }
            // Partial alias match
            if (normalizedAliases.some(alias => alias.includes(normalizedSearch) || normalizedSearch.includes(alias))) {
                return 'alias_partial';
            }
        }
        // Core match (without organizational suffixes)
        const coreSearch = EntityNormalization_1.EntityNormalization.getCoreText(searchTerm);
        const coreEntity = EntityNormalization_1.EntityNormalization.getCoreText(entityName);
     ...
     */
    static calculateMatchConfidence(searchTerm, entityName, matchType, aliases = []) {
        const normalizedSearch = EntityNormalization_1.EntityNormalization.normalizeText(searchTerm);
        const normalizedEntity = EntityNormalization_1.EntityNormalization.normalizeText(entityName);
        switch (matchType) {
            case 'exact':
            ...
            case 'alias':
                ...
                if (aliases && aliases.length > 0) {
                    const bestAliasMatch = Math.max(...aliases.map(alias => {
                        const normalizedAlias = EntityNormalization_1.EntityNormalization.normalizeText(alias);
            ...
            case 'partial':
                ...
                const wordOverlap = EntityNormalization_1.EntityNormalization.calculateWordOverlap(searchTerm, entityName);
     ...
     */
    static calculateContainmentScore(str1, str2) {
        if (!str1 || !str2)
            return 0;
     ...
     */
    static calculateMatchCoverage(searchTerm, matchedTerm, matchType) {
        if (!searchTerm || !matchedTerm)
            return 0;
        const normalizedSearch = EntityNormalization_1.EntityNormalization.normalizeText(searchTerm);
        const normalizedMatch = EntityNormalization_1.EntityNormalization.normalizeText(matchedTerm);
        switch (matchType) {
            case 'exact':
            ...
            case 'core_match':
                ...
                return EntityNormalization_1.EntityNormalization.calculateWordOverlap(searchTerm, matchedTerm);
            case 'fuzzy':
            case 'partial':
                // For fuzzy/partial, use the best available similarity metric
                return Math.max(this.jaroWinklerSimilarity(normalizedSearch, normalizedMatch), EntityNormalization_1.EntityNormalization.calculateWordOverlap(searchTerm, matchedTerm), this.calculateContainmentScore(normalizedSearch, normalizedMatch));
            default:
                return 0.5;
        }
    }
     ...
     */
    static areEntitiesEquivalent(entity1, entity2, threshold = 0.9) {
        if (entity1 === entity2)
            return true;
        const normalized1 = EntityNormalization_1.EntityNormalization.normalizeText(entity1);
        const normalized2 = EntityNormalization_1.EntityNormalization.normalizeText(entity2);
        if (normalized1 === normalized2)
            return true;
        // Check core versions
        const core1 = EntityNormalization_1.EntityNormalization.getCoreText(entity1);
        const core2 = EntityNormalization_1.EntityNormalization.getCoreText(entity2);
        if (core1 === core2 && core1.length > 3)
            return true;
        // Use similarity metrics
        const jaroWinkler = this.jaroWinklerSimilarity(normalized1, normalized2);
        const wordOverlap = EntityNormalization_1.EntityNormalization.calculateWordOverlap(entity1, entity2);
        return jaroWinkler >= threshold || wordOverlap >= threshold;
    }
}
exports.TextMatching = TextMatching;
//# sourceMappingURL=TextMatching.js.map