...
exports.QualityAssessment = void 0;
const EntityNormalization_1 = require("./EntityNormalization");
const TextMatching_1 = require("./TextMatching");
class QualityAssessment {
    /**
     * Calculate comprehensive quality metrics for a match
     */
    static calculateQualityMetrics(searchEntity, matchedEntity, matchType, context) {
        const specificityScore = EntityNormalization_1.EntityNormalization.getSpecificityScore(searchEntity);
        const lengthRatio = TextMatching_1.TextMatching.calculateLengthRatio(searchEntity, matchedEntity);
        const wordCountRatio = TextMatching_1.TextMatching.calculateWordCountRatio(searchEntity, matchedEntity);
        const matchCoverage = TextMatching_1.TextMatching.calculateMatchCoverage(searchEntity, matchedEntity, matchType);
     ...
     */
    static calculateMatchQuality(searchEntity, matchedEntity, matchType, context) {
        if (!searchEntity || !matchedEntity)
            return 0;
        ...
        if (context && metrics.context_relevance !== undefined && 'context_relevance' in weights) {
            quality += metrics.context_relevance * weights.context_relevance;
        }
     ...
     */
    static applyMatchTypeAdjustments(baseQuality, matchType, metrics) {
     ...
     */
    static calculateContextRelevance(searchEntity, matchedEntity, context) {
        if (!context)
            return 0.5; // Neutral score if no context
        const contextKeywords = EntityNormalization_1.EntityNormalization.extractKeywords(context, 3);
        const entityKeywords = EntityNormalization_1.EntityNormalization.extractKeywords(matchedEntity, 3);
        const searchKeywords = EntityNormalization_1.EntityNormalization.extractKeywords(searchEntity, 3);
     ...
     */
    static passesQualityThreshold(searchEntity, matchedEntity, matchType, context) {
     ...
     */
    static isLikelyGeographicFalsePositive(match, originalEntity) {
        const quality = this.calculateMatchQuality(originalEntity, match.organization_name, match.match_type);
        const specificityScore = EntityNormalization_1.EntityNormalization.getSpecificityScore(originalEntity);
        ...
        if (genericGeographicTerms.some(term => normalizedOriginal.includes(term) || normalizedMatch.includes(term))) {
     ...
     */
    static filterMatchResults(matches, originalEntity, context) {
        return matches
            .map(match => {
            // Calculate quality metrics for each match
            const qualityMetrics = this.calculateQualityMetrics(originalEntity, match.organization_name, match.match_type, context);
            const quality = this.calculateMatchQuality(originalEntity, match.organization_name, match.match_type, context);
            return {
                ...match,
            ...
            if (match.match_type === 'partial' || match.match_type === 'fuzzy') {
                const overlap = EntityNormalization_1.EntityNormalization.calculateWordOverlap(match.organization_name, originalEntity);
                if (overlap < 0.5) {
                    return false;
                }
            }
     ...
     */
    static getMatchTypePriority(matchType) {
        const priorities = {
            exact: 1,
            alias: 2,
            alias_partial: 3,
     ...
     */
    static shouldFilterAcademicTerm(entity, matches, maxResults = 20) {
        // Check if it's an academic term
        if (!EntityNormalization_1.EntityNormalization.isAcademicTerm(entity)) {
            return false;
        }
        ...
        const normalizedEntity = EntityNormalization_1.EntityNormalization.normalizeText(entity);
     ...
     */
    static getQualityAssessmentSummary(searchEntity, matches, context) {
        ...
        return {
            ...
            specificity_score: EntityNormalization_1.EntityNormalization.getSpecificityScore(searchEntity),
            has_context: !!context
        };
    }
}
...
QualityAssessment.QUALITY_THRESHOLDS = {
    exact: 0.3,
    alias: 0.4,
    alias_partial: 0.5,
    core_match: 0.5,
...
QualityAssessment.ALGORITHM_WEIGHTS = {
    specificity: 0.35,
    length_ratio: 0.15,
    word_coverage: 0.25,
    match_coverage: 0.25
...
QualityAssessment.CONTEXT_WEIGHTS = {
    specificity: 0.3,
    length_ratio: 0.1,
    word_coverage: 0.2,
    match_coverage: 0.2,
...
//# sourceMappingURL=QualityAssessment.js.map