...
class SSEService {
    constructor() {
        ...
        this.CONNECTION_TIMEOUT_MS = 60000; // 1 minute
     ...
     */
    createConnection(response, executionId, userId) {
     ...
     */
    sendProgress(executionId, current, total, message, apiIndex, currentEntity) {
        const event = {
            stage: 'progress',
            status: 'running',
     ...
     */
    sendNewResult(executionId, result, current, total, apiIndex) {
     ...
     */
    generateSimpleSummary(relationshipType, riskItem) {
     ...
     */
    sendCompletion(executionId, totalResults, processingTimeMs) {
        const event = {
            stage: 'completed',
            status: 'success',
     ...
     */
    sendError(executionId, error, current, total) {
     ...
     */
    sendMessage(executionId, stage, status, message, data) {
        const event = {
            stage,
            status,
            message,
     ...
     */
    getExecutionConnections(executionId) {
