...
exports.BrightDataSerpService = void 0;
const axios_1 = __importDefault(require("axios"));
const cheerio = __importStar(require("cheerio"));
class BrightDataSerpService {
    constructor() {
    ...
    selectEngines(location, riskCategory, _languages) {
    ...
    async searchSingleEngine(engine, query, options = {}) {
        try {
    ...
    buildEngineSpecificRequest(engine, searchUrl, options) {
     ...
     */
    convertToGoogleDateFormat(yearMonth, isEndDate) {
        ...
        switch (engine) {
            case 'google':
                ...
                if (options.num_results)
                    params.set('num', options.num_results.toString());
                ...
                if (options.safe_search)
                    params.set('safe', options.safe_search);
                break;
            case 'baidu':
                params.set('wd', query); // Baidu uses 'wd' instead of 'q'
                if (options.num_results)
                    params.set('rn', options.num_results.toString());
    ...
    parseEngineResponse(engine, data) {
        ...
        if (engine === 'google' && data.body) {
            ...
            if (googleBody && typeof googleBody === 'object') {
                ...
                if (organicResults && Array.isArray(organicResults)) {
                    console.log(`✅ Found ${organicResults.length} Google results in field:`, googleBody.organic ? 'organic' :
                        googleBody.results ? 'results' :
                            googleBody.search_results ? 'search_results' : 'items');
                    const parsedResults = organicResults.map((result) => ({
                        type: 'organic',
    ...
    parseGenericResponse(engine, data) {
    ...
    async searchMultipleEngines(request) {
        ...
        const selectedEngines = request.engines || this.selectEngines(request.location || 'Global', 'unknown', [request.language || 'en']).engines;
...
exports.BrightDataSerpService = BrightDataSerpService;
//# sourceMappingURL=BrightDataSerpService.js.map