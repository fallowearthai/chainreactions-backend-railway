...
exports.SerpExecutorService = void 0;
const BrightDataSerpService_1 = require("../BrightDataSerpService");
const ResultOptimizationService_1 = require("./ResultOptimizationService");
class SerpExecutorService {
    constructor() {
        this.serpService = new BrightDataSerpService_1.BrightDataSerpService();
        this.optimizationService = new ResultOptimizationService_1.ResultOptimizationService();
    }
    async executeSearchStrategy(request, metaPromptResult) {
     ...
     */
    async executeSearchStrategyOptimized(request, metaPromptResult) {
        try {
     ...
     */
    async executeSearchStrategyOptimizedWithProgress(request, metaPromptResult, progressCallback) {
        try {
     ...
     */
    async executeSearchStrategyWithProgress(request, metaPromptResult, progressCallback) {
        const startTime = Date.now();
    ...
    generateSearchTasks(metaPromptResult) {
        const tasks = [];
     ...
     */
    validateAndFormatCountryCode(countryCode, engine) {
        if (!countryCode) {
    ...
    calculateKeywordPriority(keyword, metaPromptResult) {
        let priority = 5;
        ...
        const hasEntityWord = [...entityAWords, ...entityBWords].some(word => word.length > 3 && keywordLower.includes(word));
        if (hasEntityWord) {
            priority += 3;
        }
    ...
    async executeSearchesWithConcurrency(tasks, concurrency) {
        const results = [];
        const failedTasks = [];
    ...
    async executeEngineTasksBatch(tasks, concurrency) {
        const successful = [];
        const failed = [];
        // Process in batches
    ...
    async executeSingleSearchWithTimeout(task, timeout) {
    ...
    async retryFailedTasks(failedTasks) {
        const retryResults = [];
    ...
    async executeSingleSearch(task) {
        const searchStartTime = Date.now();
        try {
            console.log(`🔍 Searching "${task.keyword}" on ${task.engine}`);
            const searchResult = await this.serpService.searchSingleEngine(task.engine, task.keyword, {
    ...
    consolidateResults(serpResults, metaPromptResult) {
        const allResults = [];
        ...
        for (const serpResult of serpResults) {
            for (const result of serpResult.results) {
                ...
                if (this.isImageSearchResult(url))
                    continue; // Skip image search results
                // Add basic metadata
                allResults.push({
                    ...result,
                    searchMetadata: {
...
- Engine Success Rates: ${Object.entries(executionSummary.performanceMetrics.engineSuccessRates)
     ...
     */
    generateCustomKeywordCombinations(metaPromptResult) {
...
exports.SerpExecutorService = SerpExecutorService;
//# sourceMappingURL=SerpExecutorService.js.map