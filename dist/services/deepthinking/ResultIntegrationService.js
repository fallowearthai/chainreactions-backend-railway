...
exports.ResultIntegrationService = void 0;
const GeminiService_1 = require("./GeminiService");
class ResultIntegrationService {
    constructor() {
        // 统一的OSINT分析师System Prompt常量
        this.OSINT_ANALYST_SYSTEM_PROMPT = `You are an expert OSINT analyst specializing in mapping institutional relationships and risk exposure. Your assignment is to examine search results and linked urls to determine the nature of relationships between specified target institutions and risk entities.
...
Prioritize factual accuracy, source attribution, and clarity in your analysis. Do not speculate or infer beyond the evidence presented.`;
     ...
     */
    async integrateAndAnalyzeOptimized(request, metaPromptResult, optimizedResults) {
        try {
            ...
            for (const riskEntity of riskEntities) {
                const entityAnalysis = await this.analyzeEntityRelationshipOptimized(request, riskEntity, metaPromptResult, optimizedResults);
     ...
     */
    cleanAndRepairJsonResponse(rawResponse) {
     ...
     */
    extractJsonAnswerFromParts(parts) {
     ...
     */
    parseJsonWithFallback(rawResponse, entityName) {
     ...
     */
    async analyzeEntityRelationshipOptimized(request, riskEntity, metaPromptResult, optimizedResults) {
        ...
        const analysisPrompt = this.buildOptimizedAnalysisPrompt(request, riskEntity, relevantResults, metaPromptResult);
        ...
        try {
            ...
            const response = await this.geminiService.generateContent([{
                ...
                }], systemInstruction, tools, // ✅ ENABLED: urlContext now active with correct specification
            {
                temperature: 0,
                thinkingConfig: {
...
- Total sources: ${result.sources?.length || 0}
    ...
    buildOptimizedAnalysisPrompt(request, riskEntity, relevantResults, _metaPromptResult) {
        const resultsText = relevantResults
            .map((result, index) => `[${index + 1}] [Score: ${result.relevanceScore}] ${result.title}\n${result.snippet}\nURL: ${result.url}\nEngine: ${result.engine}\nKeywords: ${result.searchKeywords.join(', ')}\n`).join('\n');
...
TASK: Analyze the search results above and determine the relationship between the target institution and risk entity. Return your analysis as JSON following the structure defined in the system instruction.`;
    ...
    formatSearchResultOptimized(analysisResults, optimizedResults) {
        // Combine all findings into the expected format
        const combinedFindings = analysisResults.map(result => ({
        ...
        const allSources = [...new Set(analysisResults.flatMap(result => result.sources))].filter(source => source);
...
exports.ResultIntegrationService = ResultIntegrationService;
//# sourceMappingURL=ResultIntegrationService.js.map