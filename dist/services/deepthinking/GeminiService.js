...
class GeminiService {
    ...
    async generateContent(contents, systemInstruction, tools, generationConfig) {
        try {
            const request = {
            ...
            for (let attempt = 1; attempt <= maxRetries; attempt++) {
                try {
                    const response = await this.apiClient.post(`/models/${this.model}:generateContent`, request);
    ...
    async generateSearchContent(prompt, systemInstruction) {
        const contents = [
            {
                role: 'user',
                parts: [{ text: prompt }]
    ...
    async verifyCompanyEntity(companyName, location, targetInstitution) {
...
CRITICAL: Return ONLY the JSON object. No additional text, explanations, or commentary.` }]
            ...
            const response = await this.generateContent([{ role: 'user', parts: [{ text: userPrompt }] }], systemInstruction, [{ googleSearch: {} }], {
                temperature: 0.1,
