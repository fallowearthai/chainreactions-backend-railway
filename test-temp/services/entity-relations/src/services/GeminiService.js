...
class GeminiService {
    ...
    async verifyCompanyEntity(companyName, location, targetInstitution, timeRange) {
...
\`\`\``
            ...
            const response = await this.generateContent([{ role: 'user', parts: [{ text: userPrompt }] }], systemInstruction, [
                {
                    codeExecution: {}
                },
                {
