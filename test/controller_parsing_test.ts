...
import { StandardSearchController } from '../services/entity-relations/src/controllers/StandardSearchController';
import { DeepThinkingSearchController } from '../services/entity-relations/src/controllers/DeepThinkingSearchController';
...
async function demonstrateControllerParsing() {
  ...
  const standardController = new StandardSearchController();
  const deepthinkingController = new DeepThinkingSearchController();
  ...
  try {
    ...
    try {
      ...
      if (response.candidates && response.candidates[0]?.content?.parts) {
        ...
        const textPart = parts.find((part: any) => part.text && part.text.trim().length > 0);
    ...
    const standardSearchResult = standardController['formatOptimizedSearchResults'](
      standardSearchParsed,
      standardProcessingTime
    );

    // Apply DeepThinkingSearchController formatting
    const deepthinkingSearchResult = deepthinkingController['formatOptimizedSearchResults'](
      deepthinkingSearchParsed,
      deepthinkingProcessingTime
    );
