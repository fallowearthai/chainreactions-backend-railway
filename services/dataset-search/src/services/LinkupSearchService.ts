...
// Simple API monitor for standalone service
const linkupAPIMonitor = {
  recordCall: (method: string, service: string, success: boolean) => {
    if (success) {
      logger.debug(`API call successful: ${method} for ${service}`);
    } else {
      logger.warn(`API call failed: ${method} for ${service}`);
    }
  }
};
