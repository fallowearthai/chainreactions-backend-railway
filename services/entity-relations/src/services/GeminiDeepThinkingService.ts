...
export class GeminiDeepThinkingService {
...
- Exclude speculation and general background`;
   ...
   * FIXED: Restore sources processing for frontend UI components while keeping grounding metadata for citations
   */
  private buildEnhancedResponse(
    ...
    // Process grounding chunks into sources array for frontend UI components
    const sources = groundingMetadata.grounding_chunks.map((chunk, index) => ({
      title: chunk.web?.title || `Source ${index + 1}`,
      url: chunk.web?.uri || '',
      chunk_index: index
    }));

    // Process key evidence from grounding supports for frontend inline citations
    const keyEvidence = groundingMetadata.grounding_supports.map((support) => ({
      text: support.segment?.text || '',
      source_indices: support.groundingChunkIndices || []
    }));
    ...
    const evidenceCount = keyEvidence.length;
    const coveragePercentage = sourceCount > 0 ? Math.min(100, (evidenceCount / sourceCount) * 100) : 0;

    // Build enhanced findings with proper sources and key_evidence for frontend compatibility
    const enhancedFindings = parsedFindings.map((finding, findingIndex) => {
      return {
        ...finding,
        // Restore key_evidence processing - needed for frontend inline citations
        key_evidence: keyEvidence,
        // Restore sources processing - needed for frontend UI components (SourcesTab, ResultSourcesButton)
