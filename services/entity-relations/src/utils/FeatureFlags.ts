...
export class FeatureFlags {
  private static config: GroundingConfig = {
    ...
    logLevel: (process.env.GROUNDING_LOG_LEVEL as any) || 'info'
  };
   ...
   */
  static setCurrentRequest(request: any): void {
   ...
   */
  static shouldUseEnhancedGrounding(): boolean {
   ...
   */
  static getGroundingConfig(): GroundingConfig {
    return { ...this.config };
  }
   ...
   */
  static updateGroundingConfig(updates: Partial<GroundingConfig>): void {
   ...
   */
  static log(message: string, level: 'debug' | 'info' | 'warn' | 'error' = 'info', data?: any): void {
   ...
   */
  static calculateGroundingMetrics(
    groundingChunks: any[],
   ...
   */
  static logGroundingMetrics(metrics: GroundingMetrics, requestContext: any): void {
   ...
   */
  static meetsQualityThresholds(metrics: GroundingMetrics): boolean {
    return (
      metrics.groundingChunksCount >= 5 && // At least 5 grounding chunks
   ...
   */
  private static hashRequest(request?: any): number {
    if (!request) return 0;
   ...
   * Calculate source quality score based on domain authority
   */
  private static calculateSourceQualityScore(groundingChunks: any[]): number {
    if (groundingChunks.length === 0) return 0;

    const domainScores = groundingChunks.map(chunk => {
      const uri = chunk.web?.uri || '';
      if (uri.includes('.edu')) return 1.0; // Academic: highest quality
      if (uri.includes('.gov')) return 0.95; // Government: very high quality
      if (uri.includes('.org')) return 0.8; // Organizations: high quality
      if (uri.includes('news.') || uri.includes('.news')) return 0.75; // News: good quality
      if (uri.includes('.com')) return 0.6; // Commercial: medium quality
   ...
   */
  private static logFeatureFlagChange(action: string, details: any): void {
   ...
   */
  static getFeatureFlagStatus(): any {
    return {
      enhanced_grounding: {
