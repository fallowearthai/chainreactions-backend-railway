...
export class EnhancedSearchController {
  ...
  async enhancedSearch(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      // Stage 2: Execute OPTIMIZED SERP searches with result optimization
    ...
    try {
      ...
      sendEvent({
        ...
        result: {
          ...
          sample_results: optimizedResults.consolidatedResults.map((result: any) => ({
            title: result.title,
            url: result.url,
          ...
          top_sources: [...new Set(optimizedResults.consolidatedResults.map((r: any) => {
            try {
