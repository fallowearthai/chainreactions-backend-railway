...
const CONFIG = {
  ...
  testEntity: 'Henan University',
  testLocation: 'China',
  testRuns: 3 // 每个测试运行3次取平均值
};
...
class PerformanceMonitor {
  ...
  endTimer(name) {
    if (this.metrics[name]) {
      ...
      const duration = Number(endTime - this.metrics[name].startTime) / 1000000; // 转换为毫秒
...
class DatasetMatchingPerformanceTest {
  ...
  printResults(result) {
    ...
    console.log(`🔍 Entity Search: ${result.entitySearch.metrics?.totalDuration.toFixed(2) || 'N/A'}ms`);
    console.log(`🎯 直接匹配: ${result.directMatching.metrics?.totalDuration.toFixed(2) || 'N/A'}ms`);
    console.log(`🔗 关联匹配: ${result.affiliatedMatching.metrics?.totalDuration.toFixed(2) || 'N/A'}ms`);
