...
// Note: DatasetSearchController moved to standalone service (Port 4001)
...
import { initializeMonitoring, monitoringRoutes } from './monitoring';
...
app.use(cors({
  origin: process.env.NODE_ENV === 'production'
    ...
    : [
        ...
        'http://localhost:3000', // API Gateway (Phase 2)
        'http://localhost:3001', // Development monitoring frontend
        'http://localhost:4000', // Main app (self)
        'http://localhost:4001', // Dataset search service
        'http://localhost:4002', // Entity relations service
        'http://localhost:4003'  // Dataset matching service
      ],
  credentials: true
}));
...
// Note: Dataset Search service moved to standalone service (Port 4001)
// Routes are now handled by the API Gateway -> dataset-search service
...
app.get('/api/health', healthCheckRateLimiter, async (req, res) => {
  try {
    ...
    // Check new services health (excluding dataset-search which is now standalone)
    ...
    // Overall service health (excluding standalone dataset-search)
    ...
    res.json({
      ...
      service: 'ChainReactions Main Application (Phase 2)',
      version: '3.1.0',
      ...
      phase2_note: '🔄 Now running on port 4000 as part of API Gateway architecture (Phase 2)',
      unified_services: {
        entity_relations: {
          modes: {
        ...
        // Note: Dataset search moved to standalone service (Port 4001)
        // Access via API Gateway: http://localhost:3000/api/dataset-search/*
        email_service: {
  ...
  res.json({
    ...
    services: {
      ...
      dataset_search: {
        ...
        description: 'Dataset search with SSE streaming and dual Linkup API processing (Standalone on Port 4001)',
        note: '⚡ Now running as standalone service - Access via API Gateway',
        endpoints: {
    ...
    port_unification: {
      description: 'Phase 2 Architecture: Main services unified on port 4000, standalone services accessible via API Gateway',
      phase2_architecture: {
        main_app: {
          port: 4000,
          services: ['entity_relations', 'entity_search', 'dataset_matching', 'data_management', 'email_service']
        },
        standalone_services: {
          dataset_search: {
            port: 4001,
...
if (require.main === module) {
  app.listen(PORT, '0.0.0.0', () => {
    logger.info('🚀 ChainReactions Main Application - Phase 2');
    logger.info(`📡 Server running on port ${PORT} (0.0.0.0)`);
    logger.info(`🏥 Health: http://localhost:${PORT}/api/health`);
    logger.info(`📊 Log Level: ${logger.getCurrentLevelName()}`);
    logger.info('🔄 Phase 2: Running as main application behind API Gateway');
    logger.info('🔗 API Gateway: http://localhost:3000 (when active)');

    // Initialize monitoring system
    try {
      initializeMonitoring();
      logger.info('📈 Monitoring system initialized successfully');
      logger.info(`🔍 Monitoring API: http://localhost:${PORT}/api/monitoring/health`);
    } catch (error) {
    ...
    if (!logger.isProductionMode()) {
      ...
      console.log('');
      console.log('🔍 Dataset Search Service (Standalone on Port 4001):');
      console.log(`    💡 Access via API Gateway: http://localhost:3000/api/dataset-search/*`);
      ...
      console.log(`    GET /api/monitoring/health - Get all services health status`);
      console.log(`    GET /api/monitoring/health/summary - Get system health summary`);
      console.log(`    GET /api/monitoring/health/:serviceName - Get specific service health`);
      console.log(`    POST /api/monitoring/health/check - Trigger manual health check`);
      console.log(`    GET /api/monitoring/status - Get monitoring system status`);
      console.log(`    GET /api/monitoring/enhanced-health - Enhanced health check with monitoring data`);
      console.log('');
    }
